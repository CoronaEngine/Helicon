...
enum Token_type
{
	INTEGER, PLUS, MINUS, DIV, MUL, LPAREN, RPAREN, ID, ASSIGN, SEMI, RESERVED, COMMA,XOR,
...
public:
	...
	char peek()
	{
		...
		if (pos >= text.length())
			...
			return text[pos] == ' ' ? position++,peek() : text[pos] ;
	}
...
public:
	...
	AST* Assign_var()
	{
		...
		while (current_token.type != SEMI && current_token.type != TOKEN_EOF)
		{
			Variable* node = variable();
			std::string left = node->name;
			if (GLOBAL_SCOPE.find(left) == GLOBAL_SCOPE.end())
			{
				std::cout << "Error: Variable '" << left << "' is not defined." << std::endl;
			}
	...
	AST* factor()
	{
		...
		if (token.type == ID)
		{
			std::string var_name = token.var;

			if (current_decl_type != NONE || lexer.peek() != '=')
			{
				Eat(ID);
